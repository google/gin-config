...
def fake_train_model(optimizer, scheduler=None):
  ...
  opt = optimizer([torch.rand(10), torch.rand(10)])
  # pylint: enable=E1101
  sch = None
  if scheduler:
    sch = scheduler(opt)
...
class PyTorchConfigTest(absltest.TestCase):
  ...
  def testOptimizersWithDefaults(self):
    ...
    for optimizer in optimizers:
      config.clear_config()
      config_lines = ["""fake_train_model.optimizer = @{optimizer}
                            {optimizer}.lr = 0.001"""
                        .format(optimizer=optimizer.__name__)]
      config.parse_config(config_lines)

      # pylint: disable=no-value-for-parameter
      configed_optimizer, _ = fake_train_model()
