...
import gin
from gin.config import GinState
...
class GinStateTest(absltest.TestCase):

  def tearDown(self):
    gin.config.clear_config(clear_constants=True)
    gin.config._set_config_is_locked(False)
    super(GinStateTest, self).tearDown()
  ...
  def test_from_global(self):
    gin.bind_parameter('f.x', 'global')
    with GinState(copy_state=True) as temp_state:
      self.assertEqual(gin.query_parameter('f.x'), 'global')
